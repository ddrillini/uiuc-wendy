...
def rich(sm, output):
	...
	if couples == []:
		peace("Error: There were no valid dance-routine charts found in the input file!")
	...
	for notes in couples:
		reds = notes.layers[0]
		blues = []
		for b,n in notes.layers[1]:  # We want to add a stop gimmick wherever there is a blue note in any routine chart
			new_stops.add(b)
	...
	for beat in new_stops:
		while bi < len(bpms) and beat >= bpms[bi][0]:  # Determine curbpm for the stop at "beat"
			curbpm = bpms[bi][1]
			bi += 1
...
#		usage()
	...
	output = asksaveasfilename(filetypes=[("sm files","*.sm")], initialfile=[('output.sm')])
#	if len(sys.argv) == 2:
